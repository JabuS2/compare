...
<div class="post-media-blurred" style="overflow: hidden; filter: blur(8px); pointer-events: none;">
    ...
    @endif
         ...
         <div class="col-12">
            <button class="btn btn-block to-tooltip bg-danger text-white {{(!GenericHelper::creatorCanEarnMoney($post->user)) ? 'disabled' : ''}}"
                    style="border-radius: 0; background-color: #BE1523 !important;"
                    @if(Auth::check())
                    ...
                    @endif
            >{{__('Para Desbloquear:')}} {{\App\Providers\SettingsServiceProvider::getWebsiteFormattedAmount($post->price)}}</button>
        </div>
...
.post-media-blurred {
    overflow: hidden;
    filter: blur(22px);
    pointer-events: none; /* Desativa a interação com o mouse se necessário */
}
