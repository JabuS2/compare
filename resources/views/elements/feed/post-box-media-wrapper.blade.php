...
@if($isGallery)
    @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
        <!-- Alterado para utilizar a tag img -->
    ...
    @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'video')
        <div class="video-wrapper h-100 w-100 d-flex justify-content-center align-items-center">
            <video class="video-preview w-100"controls controlsList="nodownload" preload="metadata">
              <source src="{{$attachment->path}}#t=0.001">
            </video>
        </div>
...
@else
    ...
    @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'audio')
        <div class="video-wrapper h-100 w-100 d-flex justify-content-center align-items-center">
            <audio class="video-preview w-75" src="{{$attachment->path}}#t=0.001" controls controlsList="nodownload" preload="none"></audio>
        </div>
    @endif
@endif
