...
@endphp
 ...
 @if($isGallery)
    @if(AttachmentHelper::getAttachmentType($attachment->type) == 'image')
        <div class="image-container position-relative overflow-hidden w-100 h-100 image-containerCss">
            {{-- Imagem como background --}}
            <div class="image-background position-absolute top-0 left-0 w-100 h-100 z-index-1" style="
            ...
            {{-- Imagem principal no container --}}
            <img src="{{$videoPath}}" draggable="false" alt="" class="img-fluid position-relative z-index-2 object-contain w-100">
        </div>
...
@else
    ...
    @elseif(AttachmentHelper::getAttachmentType($attachment->type) == 'video')
        ...
        <div class="video-container position-relative w-100 h-100 d-flex justify-content-center align-items-center">
            <div class="video-background-{{$attachment->id}} removebackgroundimg2 backgroundimg2" style="background-image: url('{{ Storage::url('posts/videos/thumbnails/' . $thumbnailPath . '.jpg') }}'); background-size: cover; background-position: center; filter: blur(10px) brightness(0.6);"></div>
